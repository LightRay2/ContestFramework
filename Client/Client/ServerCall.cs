...
namespace Client
{
    ...
    class ServerCall
    {
        ...
        public void Call(string serverMethod, string messageBeforeCall, string messageAfterSuccess, params object[] args)
        {
        ...
        public T Call<T>(string serverMethod, string messageBeforeCall, string messageAfterSuccess, params object[] args) 
        {
        ...
        private void Connect()
        {
            try
            {
                ...
                var task = hubProxy.Invoke<Tuple<int, string>>("AuthorizeAndGetMyId", _login, _password);
                task.Wait();
                if (string.IsNullOrEmpty(task.Result.Item2) == false )
                {
                    Message(task.Result.Item2);

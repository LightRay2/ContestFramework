    ...
    class Engine
    {
        ...
        void ProcessUnitsState(ref State state, Player player, bool reflected)
        {
            for (int i = 0; i < Const.NumberOfLines; i++)
            {
                ...
                if (unit.Type == UnitTypes.Combine)
                {
                    if (unit.PosTileX == state.Stones[i])
                    {
                        ((Combine)unit).WithStone = true;
                        EnableActive(ref state, unit);
                        state.DeleteStone(i, unit.PosTileX);
                        state.Stones[i] = -2;
                    }
                }
                ...
                switch (unit.State)
                {
                    ...
                    case UnitState.Backward:
                        if (unit.AtBase())
                        {
                            if (unit.Type == UnitTypes.Combine && ((Combine)unit).WithStone == true)
                            {
                                DisableActive(ref state, unit);
                                player.Score += Const.StoneScore;
                                ((Combine)unit).WithStone = false;
                                state.Stones[i] = -1;
                            }
        ...
        void ProcessFire (ref State state)
        {
            ...
            for (int i = 0; i < Const.NumberOfLines; i++)
            {
                ...
                if (unit1 != null && !unit1.AtBase())
                    UnitsForCollision.Add(unit1);
                if (unit2 != null && !unit2.AtBase())
            ...
            for (int i = 0; i < collisions.Count; i++)
            {
                ...
                if (collision.unit != null)
                {
                    ...
                    if (collision.unit.Hp <= 0)
                    {
                        ...
                        Effect eff = new Effect();
                        eff.AddNextStep(new DynamicObject(new Sprite(ESprite.explosion, Const.TileWidth, Const.TileHeight,
...
#region secondary classes and functions
        void EnableFire(Player player, Player enemyPlayer, ref State state)
        {
            ...
            {
                Unit unit = player.Units[i];
                if (unit == null || unit.Type == UnitTypes.Combine)
                    continue;
        ...
        struct GunRotate
        {
            public double Position; //in degrees

...
namespace SoccerPlayers
{
    public class NormalLine
    {
        ...
        void Go()
        {
            ...
            aim = we.Select(x => new Point { x = x.x, y = x.y }).ToList();
            ...
            if (weHaveBall)
            {
                var beforeus = new Point { x = ball.x + 7, y = ball.y };
                if (enemy.Min(x => Dist(x, beforeus)) < 5)
                {
                    if (memory == 0)
                    {//up

                        aim = we.Select(x => new Point { x = x.x, y = x.y - 5 }).ToList();
                    }
                    else
                    {
                        aim = we.Select(x => new Point { x = x.x, y = x.y + 5 }).ToList();
            ...
            else
            {
                for (int i = 0; i < 5; i++)
                {
                    aim[i] = new Point { x = 10 + i * 15, y = ball.y };
                }
            }
